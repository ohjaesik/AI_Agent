              ...
              {analysisResult && (
                <div className="col-12 card" style={{ border: analysisResult.status === 'interrupted' ? '2px solid var(--color-warning)' : '1px solid var(--border-color)' }}>
                  ...
                  {analysisResult.status === 'interrupted' ? (
                    ...
                    <div>
                      <p style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '16px' }}>
                        분석이 성공적으로 완료되었습니다. 최적의 AI Agent PoC 과제들이 선정되고 한글 정식 보고서가 생성되었습니다.
                      </p>
                      
                      {analysisResult.report_docx_path && (
                        <div style={{ padding: '16px', borderRadius: '8px', backgroundColor: 'var(--bg-tertiary)', border: '1px solid var(--border-color)', marginBottom: '16px' }}>
                          <span style={{ fontSize: '14px', fontWeight: 'bold' }}>생성된 DOCX 보고서 다운로드 경로:</span>
                          <code style={{ display: 'block', wordBreak: 'break-all', marginTop: '6px' }}>
                            {analysisResult.report_docx_path}
                          </code>
                        </div>
                      )}
