<!-- 파일 역할: AX Delivery Planner 전체 소개, 구조, 실행 방법, 과제 대응 내용을 설명하는 루트 문서다. -->
...
Supervisor는 장기 목표(`supervisor_long_term_goal`)와 자율 정책(`supervisor_autonomy_policy`)을 state에 고정하고, 각 stage가 handoff하기 전에 산출물 부족, Expert Agent reflection, 비용 예산, loop 상한을 함께 판단한다. 기본 base loop는 2회이고, 자율 extra loop가 켜져 있으면 stage별 최대 4회까지 bounded 반복할 수 있다.
...
반복은 자동이지만 무한 반복은 아니다. Supervisor가 `agent_autonomy_loop_decisions`에 `iterate`, `handoff`, `loop_limit_reached`, `stop_for_cost_budget` 같은 결정을 남긴다. 이전처럼 보수적으로 한 번의 handoff 흐름만 보고 싶으면 실행 시 명시적으로 끌 수 있다.

```bash
python -m app.main --project-id <PROJECT_ID> --auto-approve --disable-agent-extra-loop --verbose
```

장기 목표를 직접 지정하려면 다음처럼 실행한다.

```bash
python -m app.main --project-id <PROJECT_ID> --auto-approve --supervisor-goal "근거 부족 후보까지 최대한 자동 보강해 제출 가능한 PoC 보고서 완성" --verbose
```
...
RAG 문서 chunking은 기본적으로 `RAG_CHUNK_STRATEGY=semantic`을 사용한다. 이 방식은 문서 라인/문장을 의미 단위 후보로 나눈 뒤, 인접 단위의 단어/2-gram cosine 유사도가 `RAG_SEMANTIC_SIMILARITY_THRESHOLD`보다 낮으면 topic boundary로 보고 chunk를 끊는다. 문장마다 embedding을 만들지는 않기 때문에 추가 API 비용은 없고, 기존 800자 recursive split보다 문장 경계를 보존한다. 문제가 있으면 `RAG_CHUNK_STRATEGY=recursive`로 기존 방식으로 되돌릴 수 있다.
...
`SUPERVISOR_AUTONOMY_ENABLED=true`이면 Supervisor는 사람 개입 없이 가능한 작업을 장기 목표 기준으로 계속 밀고 간다. 각 stage마다 필수 산출물이 비었거나 Expert Agent reflection이 반복 필요를 반환하면 bounded extra loop를 실행하고, 그 판단 근거를 `agent_autonomy_loop_decisions`에 남긴다. 단, 민감/고영향/blocked/심각한 근거 부족/최종 업무 확정은 자동 결정하지 않고 Human Review boundary로 둔다.
